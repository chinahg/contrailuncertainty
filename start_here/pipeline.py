...
already_created_datasets = False # Set to True if you have already created your APCEMM input files
already_ran_APCEMM = False # Set to True if you have already run APCEMM, skips the slurm call to run APCEMM
...
test_id = "test_30"  # Test number the met and YAML files are associated with, MUST be in the format test_X
...
if already_created_datasets == False:
    ...
    test_specifications_dict = {
        ...
        "training_runs": 20,  # Number of APCEMM runs to do for training
        "validation_runs": 10,  # Number of APCEMM runs to do for validation
        "novel_runs": 500,  # Number of APCEMM runs to do for novel inputs
        ...
        "cruise_pressure": 250,  # Pressure level of the aircraft cruise altitude in hPa
        "IC_mean_amplitude_rhi": 123,  # Initial RHi value for the aircraft in %
    }
...
# Make a list of matching files after conversion
    ...
    print("Step 1: Skipping, GRUAN and ERA5 data already downloaded...")
...
if already_added_RH == False:
    ...
    print("Step 2: Skipping, RHi and RHw already added to ERA5 data.")
...
if already_preprocessed == False:
    ...
    print("Step 3: Skipping, data already preprocessed")
...
if already_created_datasets == False:
    ...
    if already_ran_APCEMM == False:
        print("Running APCEMM...")
        ...
        for set_type in ["training", "validation"]: # Run the pipeline for both training and validation sets
            ...
            RHi_sampled, RHi_std = lib.make_surrogate_input(test_specifications, set_type) # Returns a matrix of sampled RHi values for each run [runs, timesteps]

            # Update the test specifications with the computed RHi standard deviation
            test_specifications.IC_std_rhi = float(RHi_std)
            print(f"RHi standard deviation for {set_type} set: {test_specifications.IC_std_rhi}")
            # Update the YAML file with the new RHi standard deviation
            with open(test_specifications_path+"/test_specifications.yaml", "w") as f:
                yaml.dump(test_specifications.__dict__, f)
        ...
        print("APCEMM has already been run for this test. Skipping slurm submission process...")

    #Save APCEMM input and output variables to a PCE readable format
    lib.consolidate_inputs_outputs(test_specifications, set_type="training") # Create the training sample matrix for the specified test run
    lib.consolidate_inputs_outputs(test_specifications, set_type="validation") # Create the validation sample matrix for the specified test run
    ...
    print("Step 4: Skipping, APCEMM input files and outputs already created.")
...
if already_created_PCE == False:
    ...
    print("Step 5: Skipping, surrogate model already created.")
##################################################################################################################################################
### Step 6: Run the surrogate for novel inputs
if already_run_novel_inputs == False:
    ...
    print("Step 6: Skipping, surrogate for novel inputs already run.")
