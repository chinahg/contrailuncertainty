...
mem_per_job = 17000  # Memory per CPU in MB (set a fixed value or calculate based on test_id if needed)

bash_path = "/home/chinahg/GCresearch/contrailuncertainty/LRT/LES_slicing_RF.sh"
...
for i, test_id in enumerate(test_ids):
    ...
    for j, line in enumerate(bash_lines):
        if "#SBATCH --job-name=" in line:
            bash_lines[j] = f"#SBATCH --job-name=LRT-L{test_id}\n"
        if "#SBATCH -o" in line:
