...
# Run batches of APCEMM slicing on slurm
...
mem_per_job = 150000  # Memory per CPU in MB (set a fixed value or calculate based on test_id if needed)

bash_path = "/home/chinahg/GCresearch/contrailuncertainty/LRT/APCEMM_slicing_RF.sh"
...
for i, test_id in enumerate(test_ids):
    ...
    for j, line in enumerate(bash_lines):
        if "#SBATCH --job-name=" in line:
            bash_lines[j] = f"#SBATCH --job-name=LRT-A{test_id}\n"
        if "#SBATCH -o" in line:
        ...
        if "#SBATCH --cpus-per-task=" in line:
            print(f"Setting CPUs per task to {num_cpus}")
            bash_lines[j] = f"#SBATCH --cpus-per-task={num_cpus}\n"
            
        if "#SBATCH --mem=" in line:
            print(f"Setting memory per CPU to {mem_per_job:.2f} MB")
            bash_lines[j] = f"#SBATCH --mem={mem_per_job}\n"
